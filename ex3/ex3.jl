...
#> [frontmatter]
#> title = "MPIS - Zadanie 3"
#> date = "2023-01-15"

using Markdown
using InteractiveUtils

# ╔═╡ c87ec393-c095-48ea-8484-a9389632bdc1
begin
	# biblioteki używane w tym ćwiczeniu
	using Random
	using SHA
	using Plots
	using StatsPlots
...
# ╔═╡ 5e3d7fcf-4f86-4640-9844-89fb04fe1fa1
md"""# Metody Probabilistyczne i Statystyka
Zadanie domowe 3
...
# ╔═╡ 23573972-d5f2-472d-a033-1514d34087db
md"""# Zad 1 - testy NIST"""

# ╔═╡ 5ef3684b-3ea8-4636-a843-2f998b179c5f
bits_length = 10_000_000

# ╔═╡ 0c0491ad-7416-4c5f-ae7b-22e58bc8c718
seed = 21 # for reproducibility, note that RandomDevice, always produce different results

# ╔═╡ 623bb68b-5830-4bd1-bd64-6152d679dd9f
md"To see the output, uncomment `#|> println`"

# ╔═╡ c1b3a309-ca7e-4c3f-b739-dd867217a4a8
md""" linear congruential generator (LCG)
"""

# ╔═╡ 83d4afeb-4351-4029-867c-70f02d25c2de
rand(StableRNG(seed), Bool, bits_length) .|>
UInt8 .|> string |> join #|> println

# ╔═╡ 1b293d68-b09a-457a-b556-800fd73bb0bd
md"""MersenneTwister - requires much more space to store the state ector and generate a random sequence than Xoshiro256++"""

# ╔═╡ 7e8d3997-3ea3-4489-96bd-2917e206f89b
rand(MersenneTwister(seed), Bool, bits_length) .|>
UInt8 .|> string |> join # |> println

# ╔═╡ d007142c-15a9-483b-b22d-998b1e7012ae
md"Xoshiro256++ - default Julia algorithm"

# ╔═╡ 4fc98299-65f9-4c08-b0a2-620ca758bc64
rand(Xoshiro(seed), Bool, bits_length) .|>
UInt8 .|> string |> join # |> println

# ╔═╡ da204f9a-dfa4-4a32-9af6-138504852c3d
md"RandomDevice - Entropy obtained from the operating system - the only cryptographically secure random number generator from the ones tested, always produce different results"

# ╔═╡ 6f2c01d2-a2e8-4c52-ac6c-15164eae7e1b
rand(RandomDevice(), Bool, bits_length) .|>
UInt8 .|> string |> join  #|> println

# ╔═╡ 956e461e-80b9-4782-89ff-b6983cb09059
md"Output zwrócony przez funkcję hashującą SHA-1 dla własnego nazwiska"

# ╔═╡ 9be6a855-13e2-400c-a902-7c8134055ef3
sha1("Łukomski") .|> bitstring |> join |> println

# ╔═╡ 426b8fbd-2627-4c23-8596-28db9deba856
md""" Wyniki testów NIST zostały przedstawione w poniższej tabeli, dokładne zrzuty ekranu z otrzymanymi $p$-value znajdują się na końcu dokumentu. Generatory testowano na sekwencji 10 milionów bitów każdy. Z testowanych generatorów trzy, tj. `LCG`, `Xoshiro` oraz fizyczny rng komputera, przeszły wszystkie testy, a `MersenneTwister` przeszedł prawie wszystkie testy, poza _Random Excursion Test_. Output funkcji SHA-1 dla nazwiska autora - "Łukomski" - był za krótki dla 8 z 14 testów, jednak przeszedł pomyślnie wszystkie testy dla których miał odpowiednią długość. 

Test name                      | LCG | MersnneTwister | Xoshiro | RandomDevice | SHA1|
---                            | --- | ---            | ---     | ---          | --- |
...
# ╔═╡ 1ed43835-111d-4f11-b9e6-058497951f5a
md"# Zad 2 - Błądzenie losowe na liczbach całkowitych"

# ╔═╡ 36e7cb62-09ff-4296-9a68-a76e1e456d8d
md"""
...
# ╔═╡ 15b3f2f3-61e0-4f2c-b176-ecceaad60f9a
S(N) = rand([-1,1], N) |> sum

# ╔═╡ c4c4e1b1-7f02-40d0-abfe-7280124e5a83
...
# ╔═╡ 41709659-7c9a-4415-987e-0b80172a9713
md"""Poniżej przedstawiono wykresy dystrybuanty zmiennej losowej $S_N$ dla $N \in \{5,10,15,20,25,30,100\}$ oraz rozkładu normalnego $\mathcal{N}(\mu=0, \sigma=\sqrt{N})$, który miałyby aproksymować $S_N$. 

"""

# ╔═╡ 4dc83b0e-93f2-4927-96a3-1d40ec2bae21
drunkard_walk(5,10_000)

# ╔═╡ bf500070-e186-4518-932d-9c6df2f0b8d5
drunkard_walk(10,10_000)

# ╔═╡ 37b2372f-6b34-445c-b2e6-035576b6883c
drunkard_walk(15,10_000)

# ╔═╡ e70a9e32-4cfa-4e9e-8691-5c385adf292d
drunkard_walk(20,10_000)

# ╔═╡ c7d0a4d9-a29b-4eff-b540-3d21487ef9b2
drunkard_walk(25,10_000)

# ╔═╡ fc91e6cd-91fe-4607-9455-bc3ac8704878
drunkard_walk(30,10_000)

# ╔═╡ 74d19f37-fa04-4c09-bef8-28808d1fc018
drunkard_walk(100,10_000)

# ╔═╡ acb844a6-7d3d-480b-92a7-b6316570d433
md"""
Można zauważyć, że im większe $N$, tym bardziej $S_N$ zbliża się do rozkładu normalnego, aczkolwiek nie będą one nigdy sobie równe z uwagi na fakt, że rozkład normalny jest rozkładem ciąłym, a $S_N$ dyskretnym.
...
# ╔═╡ 0ca1d441-cff3-46e9-b48e-d44172d1f167
md"# Zad 3 Błądzenie losowe na liczbach całkowitych - rozkład czasu nad osią $OX$"

# ╔═╡ 0a8a2c0c-ebef-4891-83f5-ec2c3dbc6bc9
md"""
 $P_N$ odpowiada "frakcji czasu" jaki rozważany proces ($S_N$) "spęda nad osią $OX$".
"""

# ╔═╡ 255b1d56-a45e-4f1e-99ee-f3d9b06fbdba
function frac_over_OX(N,k, nbins=20)
	SN = hcat(zeros(k,1), rand([-1,1], (k,N)))
	
	cumSN = cumsum(SN, dims=2)

	D_n = reduce(hcat,[cumSN[:,i].>0 .|| cumSN[:,i-1].>0 for i in 2:(N+1)]) # S_0 = 0

	P_N = sum(D_n, dims=2) / N

	# przedziały są [a,b), więc bez nextfloat, 1.0 zostają ucinane
	plt = histogram(
		P_N, 
...
# ╔═╡ 97334304-2526-4d0f-bd8d-86f58e13ad77
md"""
Poniżej przedstawiono unormowane wykresy aproksymowanej funkcji gęstośći prawdopodobieństwa $P_N$ dla $N\in\{100,1000,10000\}$. Na wykres naniesiono też wykres gęstośći rozkłądu arcusa sinusa o dokładnym wzorze

$$\frac{d}{dx} \frac{2}{\pi} \arcsin(\sqrt{x}) = \frac{1}{\pi\sqrt{-(x-1)x}} .$$.
"""

# ╔═╡ acf37133-d76c-4522-bead-7f883410ad27
frac_over_OX(100, 5000)

# ╔═╡ 4d4b0af9-f255-4271-afc0-ec6571e0cb00
frac_over_OX(1000, 5000)

# ╔═╡ 5f3d7239-1b0c-4f0e-a1f4-5b384a33901d
frac_over_OX(10_000, 5000)

# ╔═╡ d71bc924-7574-40f5-be76-11563ef651eb
md"
Rozkłady $P_N$ całkiem dobrze estymują gęstość prawdopodobieństwa arcusa sinusa, a im większe $N$ tym lepsza ta aproksymacja.
"

# ╔═╡ 0f63d360-7bc7-4e3e-8313-3ed213826408
md"
Fakt, że co drugi przedział ma większą gęstość na histogramie pdf $P_{100}$ można wytłumaczyć skończoną liczbą wartości jakie może przyjąć $P_{100}\in \{0, 0.02, 0.04, 0.06, 0.08, 0.1, \dots \}$ (jak pijak wejdzie nad oś $OX$ to musi wykonać tyle samo kroków, żeby zejść pod $OX$, więc przebywa tam parzystą ilość czasu). Jeśli przedział $[0,1]$ podzielimy na 20 kubełków, każdy o szerokości 0.05, to każdy z nich może przyjmować dwie ($0.\_2, 0.\_4$), lub trzy ($0.\_6, 0.\_8, 0.\_0$) z dozwolonych wartości $P_{100}$ (mogłoby też być odwotnie, w zależności od tego gdzie wpadłyby próbki z zerową częścią setną ($0.\_0$)). Na kolejnych wykresach, tj $P_{1000}$ oraz $P_{10000}$ gęstość może przyjmować wielkośći, stąd bias związany z większymi gęstościami co drugiego kubełka znika.
"

# ╔═╡ 0dad10a5-4d8a-4923-b56a-d3a7173a53e9
md"""
...
# ╔═╡ cfc33bb0-e2d4-44f6-9b6f-ba086be62b48
md"""
## Screenshots from taken NIST tests

LCG
$(PlutoUI.LocalResource("./lcg.png"))

MersenneTwister
$(PlutoUI.LocalResource("./mersennetwister.png"))

Xoshiro
$(PlutoUI.LocalResource("./xoshiro.png"))

RandomDevice
$(PlutoUI.LocalResource("./randomdevice.png"))

SHA1("Łukomski")
$(PlutoUI.LocalResource("./sha1.png"))
"""
